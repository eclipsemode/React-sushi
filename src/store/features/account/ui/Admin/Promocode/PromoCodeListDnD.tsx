...
import {getAllPromoCodes, IPromocode, PromoCodeListSize} from "@store/features/promocode/api";
...
const PromoCodeListDnD = () => {
    ...
    const {promocodesList, promocodesCount, promocodeLoadSaveProcess} = useAppSelector((state) => state.promocodeReducer);
    const [cards, setCards] = React.useState<IPromocode[]>([])

    const handleDeletePromoCode = React.useCallback( (code: string) => {
                ...
                <div onDrop={async () => {
                    ...
                    <DragAndDropItem
                        ...
                        editEvent={() => {
                            dispatch(setMaterialDialog({
                                opened: true,
                                dialogType: MaterialDialogTypes.PROFILE_ADMIN_CHANGE_PROMOCODE,
                                data: card
                            }))
                        }}/>
                </div>
    ...
    const renderSkeleton = () => <div style={{width: '100%'}}><Skeleton animation='wave' height={522} sx={{transform: 'none'}} /></div>

    return (
        <>
            <Input className={styles.search} label='Поиск по промокоду' onChangeEvent={(value) => handleSearch(value)} />
            {
                promocodeLoadSaveProcess
                ? renderSkeleton()
                : <div className={styles.card}>{cards.map((card, i) => renderCard(card, i))}</div>
            }
            <Pagination onChange={(_, page) => changePage(page)}
                        count={promocodesCount % PromoCodeListSize === 0 ? promocodesCount / PromoCodeListSize : parseInt(String(promocodesCount / PromoCodeListSize + 1))}
                        sx={{justifyContent: 'center', display: 'flex', marginTop: '10px', ' button, .MuiPaginationItem-root': { color: Colors.$rootText }}} />
        </>
    )
};
