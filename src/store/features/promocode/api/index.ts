...
export const fetchPromocodeCheck = createAsyncThunk<Awaited<any>, string, {rejectValue: any}>(
    'promocode/fetchPromocodeCheck',
...
export const getAllPromoCodes = createAsyncThunk<IPromocodeWithPagination, { page?: number, match?: string }>(
    'promocode/getAllPromoCodes',
    async ({ page = 1, match = '' }, {rejectWithValue}) => {
        try {
            const res = await $api.get(`api/promocode/get?page=${page}&size=${PromoCodeListSize}&match=${match}`);
            return res.data;
        } catch (error: any) {
            if (error.response && error.response.data.message) {
                return rejectWithValue(error.response.data.message);
...
export const createPromoCode = createAsyncThunk<IPromocode, IPromocodeCreate>(
    ...
    async ({ code, discount, type, limit }, {rejectWithValue}) => {
        try {
            const res = await $api.post('api/promocode/create', { code, discount, type, limit });
            return res.data;
        } catch (error: any) {
            if (error.response && error.response.data.message) {
                return rejectWithValue(error.response.data.message);
...
const isPendingAction = isPending(fetchPromocodeCheck, createPromoCode, deletePromocode, getAllPromoCodes, changePromoCode);
const isFulfilledAction = isFulfilled(fetchPromocodeCheck, createPromoCode, deletePromocode, getAllPromoCodes, changePromoCode);
const isisRejectedAction = isRejected(fetchPromocodeCheck, createPromoCode, deletePromocode, getAllPromoCodes, changePromoCode);
...
const promoCodeSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder
            .addCase(getAllPromoCodes.fulfilled, (state, action: PayloadAction<IPromocodeWithPagination>) => {
...
export const { setPromocodeError, setPromocode, clearPromocodeData } = promoCodeSlice.actions;
