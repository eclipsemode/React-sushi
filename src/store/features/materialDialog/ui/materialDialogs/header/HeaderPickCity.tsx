...
const HeaderPickCity = () => {
    ...
    return (
        <>
            ...
            <DialogContent>
                <List
                    sx={{width: '100%', maxWidth: 360, '& li:hover': {background: Colors.$rootTextHover}}}
                    aria-label="contacts"
                >
                    {
                        allBranches.map((branch: IBranches) => branch.name === branchName ? (
                            <ListItem key={branch.id} disablePadding>
                                <ListItemButton>
                                    <ListItemIcon>
                                        <LocationCityIcon sx={{color: Colors.$rootTextActive}}/>
                                    </ListItemIcon>
                                    <ListItemText primary={branch.name} sx={{color: Colors.$rootTextActive}}/>
                                </ListItemButton>
                            </ListItem>
                        ) : (
                            <ListItem key={branch.id} onClick={() => setBranchName(branch.name)} disablePadding>
                                <ListItemButton>
                                    <ListItemText inset primary={branch.name} sx={{color: Colors.$rootText}}/>
                                </ListItemButton>
                            </ListItem>
                        ))
                    }
