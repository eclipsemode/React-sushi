...
import {createPromoCode, getAllPromoCodes, IPromocodeCreate, PromoCodeTypeEnum} from "@store/features/promocode/api";
        ...
        <>
            ...
            <form onSubmit={handleSubmit(handleAgree)} onReset={handleDisagree}>
                <DialogContent>
                    ...
                    <Select register={register} name='type' defaultValue='disabled' className={styles.row} required validate={(value) => value !== 'disabled'}>
                        <option value='disabled' disabled>Выберите тип</option>
                        <option value={PromoCodeTypeEnum.RUB}>Рубли</option>
                        <option value={PromoCodeTypeEnum.percent}>Проценты</option>
                    </Select>
                    <Input register={register} name='discount' label='Скидка' type='number' className={styles.row}
                           required/>
                    <Input register={register} name='limit' label='Лимит' type='number' className={styles.row}/>
                    <Alert variant="filled" severity="info" sx={{
                        marginTop: '15px',
                        fontSize: '12px'
                    }}>
