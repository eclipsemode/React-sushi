...
const Agreement: React.FC<IAgreementProps> = (props) => {
  const { totalPrice, deliveryPrice, totalAmount, pizzasDiscount } =
  ...
  const calculateTotalPrice = React.useCallback(() => {
    ...
    if (promocode) {
      if (props.delivery) {
        newPrice =
          promocode.type === 'RUB'
            ? totalPrice - promocode.discount + deliveryPrice
            : totalPrice -
              (totalPrice / 100) * promocode.discount +
              deliveryPrice;
      } else {
        newPrice =
          promocode.type === 'RUB'
  ...
  const calculateTotalWithPromocode = React.useCallback(
    (price: number) => {
      let newPrice: number;
  ...
  const calculatePromocode = React.useCallback(
    (price: number) => {
      let newPrice: number;
      ...
      if (promocode?.type !== 'percent') {
        newPrice =
          totalPrice -
          pizzasDiscount -
          calculateTotalWithPromocode(calculateTotalPrice());
      } else {
        newPrice = price;
      }
      ...
      <div className={styles.root}>
              ...
              {!!promocode && (
                <div className={styles.delivery}>
                  ...
                  <span>
                    -
                    {promocode.type === 'RUB'
                      ? calculatePromocode(calculateTotalPrice()) + '₽'
                      : promocode.discount + '%'}
                  </span>
                </div>
              )}
        ...
        <div className={styles.root__agreement}>
          <Checkbox
            ...
            checked={true}
          >
            Осуществляя заказ на <a href="/">сайте</a> я подтверждаю, что
            ознакомился с правилами продажи товаров, а также cо всеми
            документами, размещенными на сайте по <a href="/">адресу</a>, и
            подтверждаю принятие правил продажи товаров на сайте в полном объеме
            без ограничений.
          </Checkbox>
          ...
          <Checkbox
            ...
            checked={true}
          >
            Осуществляя заказ на <a href="/">сайте</a> я даю свое согласие на
            сбор и обработку моих персональных данных в соответствии с политикой{' '}
