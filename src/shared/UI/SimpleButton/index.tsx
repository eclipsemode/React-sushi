...
const SimpleButton: React.FC<ISimpleButton> = ({
  ...
  const buttonStyle: SxProps = {
    background:
      variant === 'contained' ? color ?? Colors.$rootTextActive : 'none',
    color:
      variant === 'contained'
        ? Colors.$rootText
        : color
        ? color
        : Colors.$rootTextActive,
    border:
      variant === 'outlined'
        ? `2px solid ${color ? color : Colors.$rootTextActive}`
        : 'auto',
    cursor: 'pointer',
    position: 'relative',
    height: '48px',
    ...
    '&:hover': {
      ...
      background:
        variant !== 'text' ? (color ? color : Colors.$rootTextActive) : 'none',
      border:
        variant !== 'text'
          ? `2px solid ${color ? color : Colors.$rootTextActive}`
          : 'none',
