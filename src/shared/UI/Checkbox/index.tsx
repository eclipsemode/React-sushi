  ...
  return (
    <div className={styles.root} style={props.style}>
      <input
        required={required}
        {...(props.register && {
          ...props.register(props.name, { validate: props.validate }),
        })}
        onChange={(e) => {
      ...
      <label
        htmlFor={props.name}
        className={
          props.errors && props.errors[props.name] && styles.root__input_invalid
        }
      >
        <div>{props.children}</div>
      </label>
      <CheckOutlined
        style={checked ? { display: 'initial' } : { display: 'none' }}
      />
    </div>
  );
};
