...
class ValidationError extends Error {
  ...
  loginError(message: string, callback: CallableFunction): void {
    if (message === ServerErrorEnum.EMAIL_NOT_FOUND || message === ServerErrorEnum.PASSWORD_INCORRECT) {
      callback('login', { type: 'custom', message: 'Неправильный логин или пароль.' })
    }
    if (message === ServerErrorEnum.ACC_NOT_ACTIVATED) {
      callback('login', { type: 'custom', message: ServerErrorEnum.ACC_NOT_ACTIVATED })
    }
  }
}
